...
%-- Method A ------------------------------%
tic;
[tfer_A,G0_A] = tfer_A(m_star,m,d,z,prop,'Rm',Rm);
t(2) = toc;

%-- Method A, Ehara et al. ----------------%
tfer_A_Ehara = tfer_A_Ehara(m_star,m,d,z,prop,'Rm',Rm);

%-- Method B ------------------------------%
tic;
[tfer_B,G0_B] = tfer_B(m_star,m,d,z,prop,'Rm',Rm);
t(3) = toc;

%-- Method C ------------------------------%
tic;
[tfer_C,G0_C] = tfer_C(m_star,m,d,z,prop,'Rm',Rm);
t(4) = toc;

%-- Method D ------------------------------%
tic;
[tfer_D,G0_D] = tfer_D(m_star,m,d,z,prop,'Rm',Rm);
t(5) = toc;

%-- Method E ------------------------------%
if prop.omega_hat==1
    tic;
    [tfer_E,G0_E] = tfer_E(m_star,m,d,z,prop,'Rm',Rm);
    t(6) = toc;
end

%-- Method F ------------------------------%
tic;
[tfer_F,G0_F] = tfer_F(m_star,m,d,z,prop,'Rm',Rm);
t(7) = toc;
...
%-- Method A ------------------------------%
tic;
[tfer_A_pb,G0_A_pb] = tfer_A_pb(m_star,m,d,z,prop,'Rm',Rm);
t(8) = toc;

%-- Method B ------------------------------%
tic;
[tfer_B_pb,G0_B_pb] = tfer_B_pb(m_star,m,d,z,prop,'Rm',Rm);
t(9) = toc;

%-- Method E ------------------------------%
if prop.omega_hat==1
    tic;
    [tfer_E_pb,G0_E_pb] = tfer_E_pb(m_star,m,d,z,prop,'Rm',Rm);
    t(10) = toc;
end
...
%-- Method D --------------------------------%
tic;
tfer_D_diff = tfer_D_diff(m_star,m,d,z,prop,'Rm',Rm);
t(14) = toc;

%-- Method E --------------------------------%
if prop.omega_hat==1
    tic;
    tfer_E_diff = tfer_E_diff(m_star,m,d,z,prop,'Rm',Rm);
    t(15) = toc;
end

%-- Method F --------------------------------%
tic;
tfer_F_diff = tfer_F_diff(m_star,m,d,z,prop,'Rm',Rm);
t(16) = toc;


%-- Triangle approx. -----------------------%
tic;
tfer_tri = tfer_tri(m_star,m,d,z,prop,'Rm',Rm);
t(18) = toc;
